import { inject, ref, onMounted, watch, provide, watchEffect, watchPostEffect, createVNode, createTextVNode, Fragment, defineComponent, computed, onUnmounted } from "vue";
...
import { SplitSide as SplitSide2 } from "@git-diff-view/core";
...
const useId = () => inject(idSymbol);
const useDom = () => inject(domSymbol);
const useMode = () => inject(modeSymbol);
const useIsMounted$1 = () => inject(mountedSymbol);
const useFontSize = () => inject(fontSizeSymbol);
const useEnableWrap = () => inject(enableWrapSymbol);
const useEnableHighlight = () => inject(enableHighlightSymbol);
const useEnableAddWidget = () => inject(enableAddWidgetSymbol);
const useExtendData = () => inject(extendDataSymbol);
const useOnAddWidgetClick = () => inject(onAddWidgetClickSymbol);
const useSlots = () => inject(slotsSymbol);
const useWidget = () => inject(widgetStateSymbol);
const useSetWidget = () => inject(setWidgetStateSymbol);
const useIsMounted = () => {
...
const DiffSplitContentLine$1 = /* @__PURE__ */ defineComponent((props) => {
  var _a, _b, _c, _d;
...
const DiffSplitExtendLine$1 = /* @__PURE__ */ defineComponent((props) => {
...
const DiffSplitHunkLineGitHub$1 = /* @__PURE__ */ defineComponent((props) => {
...
const DiffSplitHunkLineGitLab$1 = /* @__PURE__ */ defineComponent((props) => {
  const currentHunk = computed(() => props.diffFile.getSplitHunkLine(props.index));
  const enableExpand = computed(() => props.diffFile.getExpandEnabled());
  const lineSelector = computed(() => `tr[data-line="${props.lineNumber}-hunk"]`);
  const couldExpand = computed(() => enableExpand.value && currentHunk.value && currentHunk.value.splitInfo);
...
const DiffSplitHunkLine$1 = /* @__PURE__ */ defineComponent((props) => {
...
const DiffSplitWidgetLine$1 = /* @__PURE__ */ defineComponent((props) => {
  const slots = useSlots();
...
const DiffSplitViewTable = /* @__PURE__ */ defineComponent((props) => {
...
const DiffSplitViewNormal = /* @__PURE__ */ defineComponent((props) => {
  const isMounted = useIsMounted();
  const ref1 = ref();
  const ref2 = ref();
  const styleRef = ref();
...
const DiffSplitContentLine = /* @__PURE__ */ defineComponent((props) => {
  var _a, _b;
  const setWidget = useSetWidget();
  const enableAddWidget = useEnableAddWidget();
  const enableHighlight = useEnableHighlight();
...
const DiffSplitExtendLine = /* @__PURE__ */ defineComponent((props) => {
  const extendData = useExtendData();
  const slots = useSlots();
  const oldLine = computed(() => props.diffFile.getSplitLeftLine(props.index));
  const newLine = computed(() => props.diffFile.getSplitRightLine(props.index));
...
const DiffSplitHunkLineGitHub = /* @__PURE__ */ defineComponent((props) => {
  const currentHunk = computed(() => props.diffFile.getSplitHunkLine(props.index));
  const enableExpand = computed(() => props.diffFile.getExpandEnabled());
  const couldExpand = computed(() => enableExpand.value && currentHunk.value && currentHunk.value.splitInfo);
  const currentShowExpandAll = ref(currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.endHiddenIndex - currentHunk.value.splitInfo.startHiddenIndex < composeLen);
...
const DiffSplitHunkLineGitLab = /* @__PURE__ */ defineComponent((props) => {
  const currentHunk = computed(() => props.diffFile.getSplitHunkLine(props.index));
  const enableExpand = computed(() => props.diffFile.getExpandEnabled());
  const couldExpand = computed(() => enableExpand.value && currentHunk.value && currentHunk.value.splitInfo);
  const currentShowExpandAll = ref(currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.endHiddenIndex - currentHunk.value.splitInfo.startHiddenIndex < composeLen);
...
const DiffSplitHunkLine = /* @__PURE__ */ defineComponent((props) => {
  const diffViewMode = useMode();
  return () => {
    if (diffViewMode.value === DiffModeEnum.SplitGitHub || diffViewMode.value === DiffModeEnum.Split) {
...
const DiffSplitWidgetLine = /* @__PURE__ */ defineComponent((props) => {
  const slots = useSlots();
  const widget = useWidget();
  const setWidget = useSetWidget();
  const oldLine = computed(() => props.diffFile.getSplitLeftLine(props.index));
...
const DiffSplitViewWrap = /* @__PURE__ */ defineComponent((props) => {
  const lines = ref(getSplitContentLines(props.diffFile));
  const maxText = computed(() => Math.max(props.diffFile.splitLineLength, props.diffFile.fileLineLength).toString());
...
const DiffSplitView = /* @__PURE__ */ defineComponent((props) => {
...
const DiffUnifiedContentLine = /* @__PURE__ */ defineComponent((props) => {
  var _a, _b, _c, _d;
...
const DiffUnifiedExtendLine = /* @__PURE__ */ defineComponent((props) => {
  const extendData = useExtendData();
  const slots = useSlots();
  const enableWrap = useEnableWrap();
  const unifiedItem = computed(() => props.diffFile.getUnifiedLine(props.index));
...
const DiffUnifiedHunkLine = /* @__PURE__ */ defineComponent((props) => {
...
const DiffUnifiedWidgetLine = /* @__PURE__ */ defineComponent((props) => {
  const slots = useSlots();
  const widget = useWidget();
  const setWidget = useSetWidget();
  const enableWrap = useEnableWrap();
...
const DiffUnifiedView = /* @__PURE__ */ defineComponent((props) => {
...
const DiffView = /* @__PURE__ */ defineComponent((props, options) => {
  var _a, _b;
  const getInstance = () => {
    var _a2, _b2, _c, _d, _e, _f;
    if (props.diffFile) {
...
export {
  DiffModeEnum,
  DiffView,
  SplitSide2 as SplitSide,
  version
};
//# sourceMappingURL=vue-git-diff-view.mjs.map
