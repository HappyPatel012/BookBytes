...
class DiffLine {
    ...
    originalLineNumber, oldLineNumber, newLineNumber, noTrailingNewLine = false, changes, diffChanges, plainTemplate, plainTemplateMode, syntaxTemplate, syntaxTemplateMode) {
        this.text = text;
        this.type = type;
        ...
        this.plainTemplateMode = plainTemplateMode;
        this.syntaxTemplate = syntaxTemplate;
        this.syntaxTemplateMode = syntaxTemplateMode;
    }
...
const getEnableFastDiffTemplate = () => enableFastDiffTemplate;
const setEnableFastDiffTemplate = (enable) => {
    enableFastDiffTemplate = enable;
};
const resetEnableFastDiffTemplate = () => {
...
const getPlainDiffTemplate = ({ diffLine, rawLine, operator, }) => {
    if (diffLine.plainTemplate && diffLine.plainTemplateMode === "relative")
        return;
...
const getPlainDiffTemplateByFastDiff = ({ diffLine, rawLine, operator, }) => {
...
const getSyntaxDiffTemplate = ({ diffLine, syntaxLine, operator, }) => {
    ...
    if (!syntaxLine)
        return;
    if (diffLine.syntaxTemplate && diffLine.syntaxTemplateMode === "relative")
        return;
    const changes = diffLine.changes;
...
const getSyntaxDiffTemplateByFastDiff = ({ diffLine, syntaxLine, operator, }) => {
    var _a;
    if (!syntaxLine)
        return;
    if (diffLine.syntaxTemplate && diffLine.syntaxTemplateMode === "fast-diff")
        return;
    const changes = diffLine.diffChanges;
    if (!changes || !changes.hasLineChange)
        return;
    ...
    let rangeIndex = 0;
    (_a = syntaxLine === null || syntaxLine === void 0 ? void 0 : syntaxLine.nodeList) === null || _a === void 0 ? void 0 : _a.forEach(({ node, wrapper }, index, array) => {
        var _a, _b, _c;
        template += `<span data-start="${node.startIndex}" data-end="${node.endIndex}" class="${(_b = (((_a = wrapper === null || wrapper === void 0 ? void 0 : wrapper.properties) === null || _a === void 0 ? void 0 : _a.className) || [])) === null || _b === void 0 ? void 0 : _b.join(" ")}" style="${((_c = wrapper === null || wrapper === void 0 ? void 0 : wrapper.properties) === null || _c === void 0 ? void 0 : _c.style) || ""}">`;
    ...
    if (additions.length === deletions.length) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (!addition.changes || !deletion.changes) {
                ...
                const _addition = DiffLine.prototype.clone.call(addition, getAdditionRaw(addition.newLineNumber) || addition.text || "");
                const _deletion = DiffLine.prototype.clone.call(deletion, getDeletionRaw(deletion.oldLineNumber) || deletion.text || "");
        ...
        while (this.nextLine()) {
            if (this.lineStartsWith("Binary files ") && this.lineEndsWith("differ")) {
                if (process.env.NODE_ENV === "development") {
                    console.warn("Found binary file marker in diff header, @git-diff-view/core currently does not support rendering binary diffs");
                }
            ...
            if (this.lineStartsWith("+++")) {
                if (process.env.NODE_ENV === "development" && !hasMinus) {
...
function getFile(raw, lang, theme, fileName, uuid) {
    let key = raw + "--" + "0.0.32" + "--" + theme + "--" + lang;
    if (uuid) {
        key = uuid + "--" + "0.0.32" + "--" + theme + "--" + lang;
    }
    let otherThemeKey = raw + "--" + "0.0.32" + "--" + (theme === "light" ? "dark" : "light") + "--" + lang;
    if (uuid) {
        otherThemeKey = uuid + "--" + "0.0.32" + "--" + (theme === "light" ? "dark" : "light") + "--" + lang;
    }
    if (map.has(key))
        return map.get(key);
...
class DiffFile {
    ...
    initRaw() {
        ...
        if (process.env.NODE_ENV === "development") {
            if (this._diffList.some((l) => l) && !__classPrivateFieldGet(this, _DiffFile_diffLines, "f").length) {
                console.warn("the hunks from the diff string is empty, maybe the diff string is not a valid diff string, please check the diff string format or the diff string itself");
            }
        }
    }
...
const versions = "0.0.32";

export { DefaultDiffExpansionStep, DiffFile, DiffFileLineType, DiffHunk, DiffHunkExpansionType, DiffHunkHeader, DiffLine, DiffLineType, DiffParser, File, HiddenBidiCharsRegex, SplitSide, _cacheMap, assertNever, changeDefaultComposeLength, checkCurrentLineIsHidden, checkDiffLineIncludeChange, composeLen, defaultTransform, diffChanges, disableCache, escapeHtml, getCurrentComposeLength, getDiffRange, getEnableFastDiffTemplate, getFile, getHunkHeaderExpansionType, getLang, getLargestLineNumber, getPlainDiffTemplate, getPlainDiffTemplateByFastDiff, getPlainLineTemplate, getSplitContentLines, getSplitLines, getSyntaxDiffTemplate, getSyntaxDiffTemplateByFastDiff, getSyntaxLineTemplate, getUnifiedContentLine, getUnifiedLines, isTransformEnabled, numIterator, parseInstance, processTransformForFile, processTransformTemplateContent, relativeChanges, resetDefaultComposeLength, resetEnableFastDiffTemplate, resetTransform, setEnableFastDiffTemplate, setTransformForFile, setTransformForTemplateContent, versions };
