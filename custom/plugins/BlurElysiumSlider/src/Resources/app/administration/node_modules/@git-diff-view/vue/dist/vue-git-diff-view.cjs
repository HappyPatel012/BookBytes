...
const core = require("@git-diff-view/core");
...
const mountedSymbol = Symbol();
const modeSymbol = Symbol();
const fontSizeSymbol = Symbol();
const enableWrapSymbol = Symbol();
const enableHighlightSymbol = Symbol();
const enableAddWidgetSymbol = Symbol();
const slotsSymbol = Symbol();
const extendDataSymbol = Symbol();
const onAddWidgetClickSymbol = Symbol();
const widgetStateSymbol = Symbol();
const setWidgetStateSymbol = Symbol();
const useId = () => vue.inject(idSymbol);
const useDom = () => vue.inject(domSymbol);
const useMode = () => vue.inject(modeSymbol);
const useIsMounted$1 = () => vue.inject(mountedSymbol);
const useFontSize = () => vue.inject(fontSizeSymbol);
const useEnableWrap = () => vue.inject(enableWrapSymbol);
const useEnableHighlight = () => vue.inject(enableHighlightSymbol);
const useEnableAddWidget = () => vue.inject(enableAddWidgetSymbol);
const useExtendData = () => vue.inject(extendDataSymbol);
const useOnAddWidgetClick = () => vue.inject(onAddWidgetClickSymbol);
const useSlots = () => vue.inject(slotsSymbol);
const useWidget = () => vue.inject(widgetStateSymbol);
const useSetWidget = () => vue.inject(setWidgetStateSymbol);
const useIsMounted = () => {
  const isMount = vue.ref(false);
  vue.onMounted(() => {
    isMount.value = true;
  });
  return isMount;
};
const useProvide = (props, key, keySymbol, option) => {
  const value = vue.ref((props == null ? void 0 : props[key]) || (option == null ? void 0 : option.defaultValue));
  vue.watch(
  ...
  vue.provide(keySymbol, value);
};
const useSubscribeDiffFile = (props, onUpdate) => {
...
const useTextWidth = ({
  ...
  const width = vue.ref(baseSize * text.value.length);
  const measureText = () => {
    if (!isMounted.value) return;
    width.value = getTextMeasureInstance().measure(text.value || "", font.value);
  };
  vue.watchPostEffect(measureText);
  return width;
};
const DiffSplitAddWidget = ({
  side,
  className,
  lineNumber,
  onWidgetClick,
...
const DiffString = ({
  ...
  if (changes == null ? void 0 : changes.hasLineChange) {
    ...
    if (enableTemplate && !(diffLine == null ? void 0 : diffLine.plainTemplate) && typeof core.getPlainDiffTemplate === "function") {
      core.getPlainDiffTemplate({
        diffLine,
        rawLine,
        operator
      });
    ...
    if (enableTemplate && (diffLine == null ? void 0 : diffLine.plainTemplate)) {
      ...
      }, null), isNewLineSymbolChanged === NewLineSymbol.NEWLINE && vue.createVNode("span", {
      ...
      }, [vue.createVNode(DiffNoNewLine, null, null)])]);
    } else {
      ...
      }, [str1, vue.createVNode("span", {
        "data-diff-highlight": true,
        "class": "rounded-[0.2em]",
        "style": {
          backgroundColor: operator === "add" ? `var(${addContentHighlightBGName})` : `var(${delContentHighlightBGName})`
        }
      }, [isLast ? vue.createVNode(vue.Fragment, null, [_str2, vue.createVNode("span", {
        "data-newline-symbol": true
      }, [getSymbol(isNewLineSymbolChanged)])]) : str2]), str3]), isNewLineSymbolChanged === NewLineSymbol.NEWLINE && vue.createVNode("span", {
        "data-no-newline-at-end-of-file-symbol": true,
        "class": enableWrap ? "block !text-red-500" : "inline-block align-middle !text-red-500",
        "style": {
          width: `var(${diffFontSizeName})`,
...
const DiffSyntax = ({
  ...
  var _a, _b;
  if (!syntaxLine) {
    return vue.createVNode(DiffString, {
      "rawLine": rawLine,
      "diffLine": diffLine,
      "operator": operator,
  ...
  if (changes == null ? void 0 : changes.hasLineChange) {
    const isNewLineSymbolChanged = changes.newLineSymbol;
    if (enableTemplate && !(diffLine == null ? void 0 : diffLine.syntaxTemplate) && typeof core.getSyntaxDiffTemplate === "function") {
      core.getSyntaxDiffTemplate({
        diffLine,
        syntaxLine,
        operator
      });
    ...
    if (enableTemplate && (diffLine == null ? void 0 : diffLine.syntaxTemplate)) {
      ...
      })]), isNewLineSymbolChanged === NewLineSymbol.NEWLINE && vue.createVNode("span", {
        "data-no-newline-at-end-of-file-symbol": true,
        "class": enableWrap ? "block !text-red-500" : "inline-block align-middle !text-red-500",
        "style": {
          width: `var(${diffFontSizeName})`,
...
const DiffContent = ({
  ...
  var _a, _b;
  const isAdded = (diffLine == null ? void 0 : diffLine.type) === core.DiffLineType.Add;
  const isDelete = (diffLine == null ? void 0 : diffLine.type) === core.DiffLineType.Delete;
  ...
  }, [isAdded ? "+" : isDelete ? "-" : " "]), enableHighlight && syntaxLine && !isMaxLineLengthToIgnoreSyntax ? vue.createVNode(DiffSyntax, {
  ...
  }, null) : vue.createVNode(DiffString, {
    "operator": isAdded ? "add" : isDelete ? "del" : void 0,
    "rawLine": rawLine,
    "diffLine": diffLine,
    "plainLine": plainLine,
...
const DiffSplitContentLine$1 = /* @__PURE__ */ vue.defineComponent((props) => {
  var _a, _b, _c, _d;
  ...
  const currentLine = vue.computed(() => props.side === core.SplitSide.old ? props.diffFile.getSplitLeftLine(props.index) : props.diffFile.getSplitRightLine(props.index));
  const currentLineHasDiff = vue.computed(() => {
    var _a2;
    return !!((_a2 = currentLine.value) == null ? void 0 : _a2.diff);
  });
  const currentLineHasChange = vue.computed(() => {
    var _a2;
    return core.checkDiffLineIncludeChange((_a2 = currentLine.value) == null ? void 0 : _a2.diff);
  });
  const currentLineHasHidden = vue.computed(() => {
    var _a2;
    return (_a2 = currentLine.value) == null ? void 0 : _a2.isHidden;
  });
  const currentLineHasContent = vue.computed(() => {
    var _a2;
    return (_a2 = currentLine.value) == null ? void 0 : _a2.lineNumber;
  });
  const currentSyntaxLine = vue.ref(props.side === core.SplitSide.old ? props.diffFile.getOldSyntaxLine((_a = currentLine.value) == null ? void 0 : _a.lineNumber) : props.diffFile.getNewSyntaxLine((_b = currentLine.value) == null ? void 0 : _b.lineNumber));
  const currentPlainLine = vue.ref(props.side === core.SplitSide.old ? props.diffFile.getOldPlainLine((_c = currentLine.value) == null ? void 0 : _c.lineNumber) : props.diffFile.getNewPlainLine((_d = currentLine.value) == null ? void 0 : _d.lineNumber));
  useSubscribeDiffFile(props, (diffFile) => {
    var _a2, _b2, _c2, _d2;
    currentSyntaxLine.value = props.side === core.SplitSide.old ? diffFile.getOldSyntaxLine((_a2 = currentLine.value) == null ? void 0 : _a2.lineNumber) : diffFile.getNewSyntaxLine((_b2 = currentLine.value) == null ? void 0 : _b2.lineNumber);
    currentPlainLine.value = props.side === core.SplitSide.old ? diffFile.getOldPlainLine((_c2 = currentLine.value) == null ? void 0 : _c2.lineNumber) : diffFile.getNewPlainLine((_d2 = currentLine.value) == null ? void 0 : _d2.lineNumber);
  });
  ...
  return () => {
    ...
    const isAdded = ((_b2 = (_a2 = currentLine.value) == null ? void 0 : _a2.diff) == null ? void 0 : _b2.type) === core.DiffLineType.Add;
    const isDelete = ((_d2 = (_c2 = currentLine.value) == null ? void 0 : _c2.diff) == null ? void 0 : _d2.type) === core.DiffLineType.Delete;
    ...
    return vue.createVNode("tr", {
      ...
      "data-side": core.SplitSide[props.side],
      "class": "diff-line" + (currentLineHasContent.value ? " group" : "")
    }, [currentLineHasContent.value ? vue.createVNode(vue.Fragment, null, [vue.createVNode("td", {
      "class": `diff-line-${core.SplitSide[props.side]}-num sticky left-0 z-[1] w-[1%] min-w-[40px] select-none pl-[10px] pr-[10px] text-right align-top`,
      "style": {
    ...
    }, [currentLineHasDiff.value && enableAddWidget.value && vue.createVNode(DiffSplitAddWidget, {
      "index": props.index,
    ...
    }, [currentLine.value.lineNumber])]), vue.createVNode("td", {
      "class": `diff-line-${core.SplitSide[props.side]}-content pr-[10px] align-top`,
      "style": {
        backgroundColor: contentBG
      }
    }, [vue.createVNode(DiffContent, {
      "enableWrap": false,
      "diffFile": props.diffFile,
    ...
    }, null)])]) : vue.createVNode("td", {
      "class": `diff-line-${core.SplitSide[props.side]}-placeholder select-none`,
      "style": {
        backgroundColor: `var(${emptyBGName})`
      },
      "colspan": 2
    }, [vue.createVNode("span", null, [vue.createTextVNode(" ")])])]);
  };
}, {
...
const useDomWidth = ({ selector, enable }) => {
  ...
  vue.watchPostEffect((onCancel) => observeWidth(onCancel));
  return width;
};
const useSyncHeight = ({
  selector,
  ...
  vue.watchPostEffect(observeHeight);
};
const DiffSplitExtendLine$1 = /* @__PURE__ */ vue.defineComponent((props) => {
  const extendData = useExtendData();
  const slots = useSlots();
  const lineSelector = vue.computed(() => `div[data-line="${props.lineNumber}-extend-content"]`);
  const lineWrapperSelector = vue.computed(() => `tr[data-line="${props.lineNumber}-extend"]`);
  const wrapperSelector = vue.computed(() => props.side === core.SplitSide.old ? ".old-diff-table-wrapper" : ".new-diff-table-wrapper");
  const oldLine = vue.computed(() => props.diffFile.getSplitLeftLine(props.index));
  const newLine = vue.computed(() => props.diffFile.getSplitRightLine(props.index));
  const enableExpand = vue.computed(() => props.diffFile.getExpandEnabled());
  const oldLineExtend = vue.computed(() => {
    var _a, _b, _c;
    return (_c = (_a = extendData.value) == null ? void 0 : _a.oldFile) == null ? void 0 : _c[(_b = oldLine.value) == null ? void 0 : _b.lineNumber];
  });
  const newLineExtend = vue.computed(() => {
    var _a, _b;
    return (_b = (_a = extendData.value) == null ? void 0 : _a.newFile) == null ? void 0 : _b[newLine.value.lineNumber];
  });
  const currentItem = vue.computed(() => props.side === core.SplitSide.old ? oldLine.value : newLine.value);
  const currentIsHidden = vue.computed(() => currentItem.value.isHidden);
  const currentExtend = vue.computed(() => props.side === core.SplitSide.old ? oldLineExtend.value : newLineExtend.value);
  const currentLineNumber = vue.computed(() => props.side === core.SplitSide.old ? oldLine.value.lineNumber : newLine.value.lineNumber);
  const currentIsShow = vue.computed(() => Boolean((oldLineExtend.value || newLineExtend.value) && (!currentIsHidden.value || enableExpand.value) && slots.extend));
  const currentEnable = vue.computed(() => (props.side === core.SplitSide.old ? !!oldLineExtend.value : !!newLineExtend.value) && currentIsShow.value);
  const extendSide = vue.computed(() => core.SplitSide[currentExtend.value ? props.side : props.side === core.SplitSide.new ? core.SplitSide.old : core.SplitSide.new]);
  useSyncHeight({
  ...
  return () => {
    ...
    return vue.createVNode("tr", {
    ...
    }, [currentExtend.value ? vue.createVNode("td", {
      "class": `diff-line-extend-${core.SplitSide[props.side]}-content p-0`,
      "colspan": 2
    }, [vue.createVNode("div", {
      ...
      "class": `diff-line-extend-${core.SplitSide[props.side]}-placeholder select-none p-0`,
      "style": {
        backgroundColor: `var(${emptyBGName})`
      },
      "colspan": 2
...
const DiffSplitHunkLineGitHub$1 = /* @__PURE__ */ vue.defineComponent((props) => {
  const currentHunk = vue.computed(() => props.diffFile.getSplitHunkLine(props.index));
  const enableExpand = vue.computed(() => props.diffFile.getExpandEnabled());
  const couldExpand = vue.computed(() => enableExpand.value && currentHunk.value && currentHunk.value.splitInfo);
  const lineSelector = vue.computed(() => `tr[data-line="${props.lineNumber}-hunk"]`);
  const currentShowExpand = vue.computed(() => props.side === core.SplitSide.old);
  const currentSyncHeightSide = vue.computed(() => core.SplitSide[core.SplitSide.old]);
  const currentIsFirstLine = vue.computed(() => currentHunk.value && currentHunk.value.isFirst);
  const currentIsLastLine = vue.computed(() => currentHunk.value && currentHunk.value.isLast);
  const currentIsPureHunk = vue.computed(() => currentHunk.value && props.diffFile._getIsPureDiffRender() && !currentHunk.value.splitInfo);
  const currentShowExpandAll = vue.ref(currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.endHiddenIndex - currentHunk.value.splitInfo.startHiddenIndex < core.composeLen);
  const currentIsShow = vue.ref(currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.startHiddenIndex < currentHunk.value.splitInfo.endHiddenIndex);
  useSubscribeDiffFile(props, () => {
    currentShowExpandAll.value = currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.endHiddenIndex - currentHunk.value.splitInfo.startHiddenIndex < core.composeLen;
  ...
  const currentEnableSyncHeight = vue.computed(() => props.side === core.SplitSide.new && currentIsShow.value);
  useSyncHeight({
    selector: lineSelector,
  ...
  return () => {
    ...
    return vue.createVNode("tr", {
    ...
    }, [currentShowExpand.value ? vue.createVNode(vue.Fragment, null, [vue.createVNode("td", {
    ...
    }, [couldExpand.value ? currentIsFirstLine.value ? vue.createVNode("button", {
    ...
    }, [vue.createVNode(ExpandUp, {
      "className": "fill-current"
    }, null)]) : currentIsLastLine.value ? vue.createVNode("button", {
      "class": "diff-widget-tooltip flex w-full cursor-pointer items-center justify-center rounded-[2px] py-[6px]",
      "title": "Expand Down",
      "data-title": "Expand Down",
      "onClick": () => props.diffFile.onSplitHunkExpand("down", props.index)
    }, [vue.createVNode(ExpandDown, {
      "className": "fill-current"
    }, null)]) : currentShowExpandAll.value ? vue.createVNode("button", {
      "class": "diff-widget-tooltip flex w-full cursor-pointer items-center justify-center rounded-[2px] py-[6px]",
      "title": "Expand All",
      "data-title": "Expand All",
      "onClick": () => props.diffFile.onSplitHunkExpand("all", props.index)
    }, [vue.createVNode(ExpandAll, {
      "className": "fill-current"
    }, null)]) : vue.createVNode(vue.Fragment, null, [vue.createVNode("button", {
      ...
      "class": "min-h-[28px]"
    }, [vue.createTextVNode(" ")])]), vue.createVNode("td", {
    ...
    }, [((_a = currentHunk.value.splitInfo) == null ? void 0 : _a.plainText) || currentHunk.value.text])])]) : vue.createVNode("td", {
...
const DiffSplitHunkLineGitLab$1 = /* @__PURE__ */ vue.defineComponent((props) => {
  const currentHunk = vue.computed(() => props.diffFile.getSplitHunkLine(props.index));
  const enableExpand = vue.computed(() => props.diffFile.getExpandEnabled());
  const lineSelector = vue.computed(() => `tr[data-line="${props.lineNumber}-hunk"]`);
  const couldExpand = vue.computed(() => enableExpand.value && currentHunk.value && currentHunk.value.splitInfo);
  ...
  const currentIsLastLine = vue.computed(() => currentHunk.value && currentHunk.value.isLast);
  const currentIsShow = vue.ref(currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.startHiddenIndex < currentHunk.value.splitInfo.endHiddenIndex);
  const currentSyncHeightSide = vue.computed(() => core.SplitSide[props.side]);
  const currentEnableSyncHeight = vue.computed(() => currentIsShow.value);
  useSyncHeight({
    selector: lineSelector,
    wrapper: lineSelector,
    side: currentSyncHeightSide,
...
const DiffSplitHunkLine$1 = /* @__PURE__ */ vue.defineComponent((props) => {
  const diffViewMode = useMode();
  return () => {
    if (diffViewMode.value === exports.DiffModeEnum.SplitGitHub || diffViewMode.value === exports.DiffModeEnum.Split) {
      return vue.createVNode(DiffSplitHunkLineGitHub$1, {
        "index": props.index,
        "side": props.side,
        "diffFile": props.diffFile,
      ...
      return vue.createVNode(DiffSplitHunkLineGitLab$1, {
        "index": props.index,
        "side": props.side,
        "diffFile": props.diffFile,
        "lineNumber": props.lineNumber
...
const DiffSplitWidgetLine$1 = /* @__PURE__ */ vue.defineComponent((props) => {
  const slots = useSlots();
  const widget = useWidget();
  const setWidget = useSetWidget();
  const oldLine = vue.computed(() => props.diffFile.getSplitLeftLine(props.index));
  const newLine = vue.computed(() => props.diffFile.getSplitRightLine(props.index));
  const oldLineWidget = vue.computed(() => oldLine.value.lineNumber && widget.value.side === core.SplitSide.old && widget.value.lineNumber === oldLine.value.lineNumber);
  const newLineWidget = vue.computed(() => newLine.value.lineNumber && widget.value.side === core.SplitSide.new && widget.value.lineNumber === newLine.value.lineNumber);
  const currentLine = vue.computed(() => props.side === core.SplitSide.old ? oldLine.value : newLine.value);
  const currentIsHidden = vue.computed(() => currentLine.value.isHidden);
  const lineSelector = vue.computed(() => `div[data-line="${props.lineNumber}-widget-content"]`);
  const lineWrapperSelector = vue.computed(() => `tr[data-line="${props.lineNumber}-widget"]`);
  const wrapperSelector = vue.computed(() => props.side === core.SplitSide.old ? ".old-diff-table-wrapper" : ".new-diff-table-wrapper");
  const currentWidget = vue.computed(() => props.side === core.SplitSide.old ? oldLineWidget.value : newLineWidget.value);
  const observeSide = vue.computed(() => core.SplitSide[currentWidget ? props.side : props.side === core.SplitSide.old ? core.SplitSide.new : core.SplitSide.old]);
  const currentIsShow = vue.computed(() => (!!oldLineWidget.value || !!newLineWidget.value) && !currentIsHidden.value && !!slots.widget);
  const currentEnable = vue.computed(() => currentWidget.value && !!currentIsShow.value);
  ...
  return () => {
    ...
    return vue.createVNode("tr", {
    ...
    }, [currentWidget.value ? vue.createVNode("td", {
      "class": `diff-line-widget-${core.SplitSide[props.side]}-content p-0`,
      "colspan": 2
    }, [vue.createVNode("div", {
      ...
      "class": `diff-line-widget-${core.SplitSide[props.side]}-placeholder select-none p-0`,
      "style": {
        backgroundColor: `var(${emptyBGName})`
      },
      "colspan": 2
...
const DiffSplitViewTable = /* @__PURE__ */ vue.defineComponent((props) => {
  const className = vue.computed(() => props.side === core.SplitSide.new ? "new-diff-table" : "old-diff-table");
  const lines = vue.ref(core.getSplitContentLines(props.diffFile));
  useSubscribeDiffFile(props, (diffFile) => {
    lines.value = core.getSplitContentLines(diffFile);
  });
  ...
  return () => {
    return vue.createVNode("table", {
      ...
      "data-mode": core.SplitSide[props.side]
    }, [vue.createVNode("colgroup", null, [vue.createVNode("col", {
      "class": `diff-table-${core.SplitSide[props.side]}-num-col`
    }, null), vue.createVNode("col", {
      "class": `diff-table-${core.SplitSide[props.side]}-content-col`
    }, null)]), vue.createVNode("thead", {
      "class": "hidden"
    }, [vue.createVNode("tr", null, [vue.createVNode("th", {
      "scope": "col"
    }, [core.SplitSide[props.side], vue.createTextVNode(" line number")]), vue.createVNode("th", {
      "scope": "col"
    }, [core.SplitSide[props.side], vue.createTextVNode(" line content")])])]), vue.createVNode("tbody", {
      "class": "diff-table-body leading-[1.6]",
      "onMousedown": onMouseDown
    }, [lines.value.map((item) => vue.createVNode(vue.Fragment, {
      "key": item.index
    }, [vue.createVNode(DiffSplitHunkLine$1, {
      "index": item.index,
      "side": props.side,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffSplitContentLine$1, {
      "index": item.index,
      "side": props.side,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffSplitWidgetLine$1, {
      "index": item.index,
      "side": props.side,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffSplitExtendLine$1, {
      "index": item.index,
      "side": props.side,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null)])), vue.createVNode(DiffSplitHunkLine$1, {
      "side": props.side,
...
const DiffSplitViewNormal = /* @__PURE__ */ vue.defineComponent((props) => {
  const isMounted = useIsMounted();
  const ref1 = vue.ref();
  const ref2 = vue.ref();
  const styleRef = vue.ref();
  const maxText = vue.computed(() => Math.max(props.diffFile.splitLineLength, props.diffFile.fileLineLength).toString());
  ...
  vue.watchPostEffect(initSyncScroll);
  const onSelect = (side) => {
...
#${id} [data-state="widget"] {user-select: none}`;
  ...
  const computedWidth = vue.computed(() => Math.max(40, width.value + 25));
  return () => {
    return vue.createVNode("div", {
      "class": "split-diff-view split-diff-view-normal flex w-full basis-[50%]"
    }, [vue.createVNode("style", {
    ...
    }, [vue.createVNode(DiffSplitViewTable, {
...
const DiffSplitContentLine = /* @__PURE__ */ vue.defineComponent((props) => {
  var _a, _b;
  const setWidget = useSetWidget();
  const enableAddWidget = useEnableAddWidget();
  const enableHighlight = useEnableHighlight();
  ...
  const oldLine = vue.computed(() => props.diffFile.getSplitLeftLine(props.index));
  const newLine = vue.computed(() => props.diffFile.getSplitRightLine(props.index));
  const oldSyntaxLine = vue.ref(props.diffFile.getOldSyntaxLine((_a = oldLine.value) == null ? void 0 : _a.lineNumber));
  const newSyntaxLine = vue.ref(props.diffFile.getNewSyntaxLine((_b = newLine.value) == null ? void 0 : _b.lineNumber));
  const oldPlainLine = vue.ref(props.diffFile.getOldPlainLine(oldLine.value.lineNumber));
  const newPlainLine = vue.ref(props.diffFile.getNewPlainLine(newLine.value.lineNumber));
  const hasDiff = vue.computed(() => {
    var _a2, _b2;
    return !!((_a2 = oldLine.value) == null ? void 0 : _a2.diff) || !!((_b2 = newLine.value) == null ? void 0 : _b2.diff);
  });
  const hasChange = vue.computed(() => {
    var _a2, _b2;
    return core.checkDiffLineIncludeChange((_a2 = oldLine.value) == null ? void 0 : _a2.diff) || core.checkDiffLineIncludeChange((_b2 = newLine.value) == null ? void 0 : _b2.diff);
  });
  const hasHidden = vue.computed(() => {
    var _a2, _b2;
  ...
  return () => {
    ...
    const oldLineIsDelete = ((_d = (_c = oldLine.value) == null ? void 0 : _c.diff) == null ? void 0 : _d.type) === core.DiffLineType.Delete;
    const newLineIsAdded = ((_f = (_e = newLine.value) == null ? void 0 : _e.diff) == null ? void 0 : _f.type) === core.DiffLineType.Add;
    ...
    return vue.createVNode("tr", {
      ...
      "class": "diff-line"
    }, [hasOldLine ? vue.createVNode(vue.Fragment, null, [vue.createVNode("td", {
    ...
    }, [hasDiff.value && enableAddWidget.value && vue.createVNode(DiffSplitAddWidget, {
      "index": props.index,
      ...
      "onOpenAddWidget": onOpenAddWidget
    }, null), vue.createVNode(DiffContent, {
      "enableWrap": true,
      "diffFile": props.diffFile,
      ...
      "colspan": 2
    }, [vue.createVNode("span", null, [vue.createTextVNode(" ")])]), hasNewLine ? vue.createVNode(vue.Fragment, null, [vue.createVNode("td", {
...
const DiffSplitExtendLine = /* @__PURE__ */ vue.defineComponent((props) => {
  const extendData = useExtendData();
  const slots = useSlots();
  const oldLine = vue.computed(() => props.diffFile.getSplitLeftLine(props.index));
  const newLine = vue.computed(() => props.diffFile.getSplitRightLine(props.index));
  ...
  const hasHidden = vue.computed(() => oldLine.value.isHidden && newLine.value.isHidden);
  const currentIsShow = vue.computed(() => Boolean((oldLineExtend.value || newLineExtend.value) && (!hasHidden.value || enableExpand.value) && slots.extend));
  return () => {
    var _a, _b;
    if (!currentIsShow.value) return null;
    ...
    return vue.createVNode("tr", {
      ...
      "data-state": "extend",
      "class": "diff-line diff-line-extend"
    }, [oldExtendRendered ? vue.createVNode("td", {
      ...
      "colspan": 2
    }, null), newExtendRendered ? vue.createVNode("td", {
      ...
      "class": "diff-line-extend-wrapper"
    }, [newExtendRendered])]) : vue.createVNode("td", {
...
const DiffSplitHunkLineGitHub = /* @__PURE__ */ vue.defineComponent((props) => {
  const currentHunk = vue.computed(() => props.diffFile.getSplitHunkLine(props.index));
  const enableExpand = vue.computed(() => props.diffFile.getExpandEnabled());
  const couldExpand = vue.computed(() => enableExpand.value && currentHunk.value && currentHunk.value.splitInfo);
  const currentShowExpandAll = vue.ref(currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.endHiddenIndex - currentHunk.value.splitInfo.startHiddenIndex < core.composeLen);
...
const DiffSplitHunkLineGitLab = /* @__PURE__ */ vue.defineComponent((props) => {
  const currentHunk = vue.computed(() => props.diffFile.getSplitHunkLine(props.index));
  const enableExpand = vue.computed(() => props.diffFile.getExpandEnabled());
  const couldExpand = vue.computed(() => enableExpand.value && currentHunk.value && currentHunk.value.splitInfo);
  const currentShowExpandAll = vue.ref(currentHunk.value && currentHunk.value.splitInfo && currentHunk.value.splitInfo.endHiddenIndex - currentHunk.value.splitInfo.startHiddenIndex < core.composeLen);
...
const DiffSplitHunkLine = /* @__PURE__ */ vue.defineComponent((props) => {
  const diffViewMode = useMode();
  return () => {
    if (diffViewMode.value === exports.DiffModeEnum.SplitGitHub || diffViewMode.value === exports.DiffModeEnum.Split) {
      return vue.createVNode(DiffSplitHunkLineGitHub, {
        "index": props.index,
        "diffFile": props.diffFile,
        "lineNumber": props.lineNumber
      }, null);
    } else {
      return vue.createVNode(DiffSplitHunkLineGitLab, {
        "index": props.index,
        "diffFile": props.diffFile,
        "lineNumber": props.lineNumber
      }, null);
...
const DiffSplitWidgetLine = /* @__PURE__ */ vue.defineComponent((props) => {
  const slots = useSlots();
  const widget = useWidget();
  const setWidget = useSetWidget();
  const oldLine = vue.computed(() => props.diffFile.getSplitLeftLine(props.index));
  ...
  const newLineWidget = vue.computed(() => newLine.value.lineNumber && widget.value.side === core.SplitSide.new && widget.value.lineNumber === newLine.value.lineNumber);
  const hasHidden = vue.computed(() => oldLine.value.isHidden && newLine.value.isHidden);
  const currentIsShow = vue.computed(() => (!!oldLineWidget.value || !!newLineWidget.value) && !hasHidden.value && !!slots.widget);
  const onCloseWidget = () => setWidget({});
  return () => {
    var _a, _b;
    if (!currentIsShow.value) return null;
    ...
    return vue.createVNode("tr", {
      ...
      "data-state": "widget",
      "class": "diff-line diff-line-widget"
    }, [oldWidgetRendered ? vue.createVNode("td", {
      ...
      "colspan": 2
    }, null), newWidgetRendered ? vue.createVNode("td", {
      ...
      "class": "diff-line-widget-wrapper"
    }, [newWidgetRendered])]) : vue.createVNode("td", {
...
const DiffSplitViewWrap = /* @__PURE__ */ vue.defineComponent((props) => {
  const lines = vue.ref(core.getSplitContentLines(props.diffFile));
  const maxText = vue.computed(() => Math.max(props.diffFile.splitLineLength, props.diffFile.fileLineLength).toString());
  ...
  const onSelect = (side) => {
    ...
    if (!side) {
      ...
      const id = `diff-root${props.diffFile.getId()}`;
      ele.textContent = `#${id} [data-side="${core.SplitSide[side === core.SplitSide.old ? core.SplitSide.new : core.SplitSide.old]}"] {user-select: none} 
...
#${id} [data-state="widget"] {user-select: none}`;
  ...
  const onMouseDown = (e) => {
    ...
    while (ele && ele instanceof HTMLElement) {
      ...
      if (side) {
        if (selectState.current !== core.SplitSide[side]) {
          selectState.current = core.SplitSide[side];
          onSelect(core.SplitSide[side]);
          removeAllSelection();
        }
      }
      if (state) {
  ...
  return () => {
    return vue.createVNode("div", {
      ...
      "scope": "col"
    }, [vue.createTextVNode("old line number")]), vue.createVNode("th", {
      "scope": "col"
    }, [vue.createTextVNode("old line content")]), vue.createVNode("th", {
      "scope": "col"
    }, [vue.createTextVNode("new line number")]), vue.createVNode("th", {
      "scope": "col"
    }, [vue.createTextVNode("new line content")])])]), vue.createVNode("tbody", {
      "class": "diff-table-body leading-[1.6]",
      "onMousedown": onMouseDown
    }, [lines.value.map((item) => vue.createVNode(vue.Fragment, {
      "key": item.index
    }, [vue.createVNode(DiffSplitHunkLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffSplitContentLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffSplitWidgetLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffSplitExtendLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null)])), vue.createVNode(DiffSplitHunkLine, {
      "index": props.diffFile.splitLineLength,
      "lineNumber": props.diffFile.splitLineLength,
      "diffFile": props.diffFile
    }, null)])])])]);
...
const DiffSplitView = /* @__PURE__ */ vue.defineComponent((props) => {
  const enableWrap = useEnableWrap();
  return () => {
    return enableWrap.value ? vue.createVNode(DiffSplitViewWrap, {
      "diffFile": props.diffFile
    }, null) : vue.createVNode(DiffSplitViewNormal, {
      "diffFile": props.diffFile
    }, null);
  };
}, {
...
const DiffUnifiedOldLine = ({
  ...
  }, [enableAddWidget && vue.createVNode(DiffUnifiedAddWidget, {
    "index": index - 1,
  ...
  }, [lineNumber]), vue.createVNode("span", {
...
const DiffUnifiedContentLine = /* @__PURE__ */ vue.defineComponent((props) => {
  var _a, _b, _c, _d;
  const unifiedItem = vue.computed(() => props.diffFile.getUnifiedLine(props.index));
  const enableWrap = useEnableWrap();
  const setWidget = useSetWidget();
  const onAddWidgetClick = useOnAddWidgetClick();
  const enableHighlight = useEnableHighlight();
  const enableAddWidget = useEnableAddWidget();
  const currentItemHasHidden = vue.computed(() => {
    var _a2;
    return (_a2 = unifiedItem.value) == null ? void 0 : _a2.isHidden;
  });
  const currentItemHasChange = vue.computed(() => {
    var _a2;
    return core.checkDiffLineIncludeChange((_a2 = unifiedItem.value) == null ? void 0 : _a2.diff);
  });
  const currentSyntaxLine = vue.ref(((_a = unifiedItem.value) == null ? void 0 : _a.newLineNumber) ? props.diffFile.getNewSyntaxLine(unifiedItem.value.newLineNumber) : ((_b = unifiedItem.value) == null ? void 0 : _b.oldLineNumber) ? props.diffFile.getOldSyntaxLine(unifiedItem.value.oldLineNumber) : void 0);
  const currentPlainLine = vue.ref(((_c = unifiedItem.value) == null ? void 0 : _c.newLineNumber) ? props.diffFile.getNewPlainLine(unifiedItem.value.newLineNumber) : ((_d = unifiedItem.value) == null ? void 0 : _d.oldLineNumber) ? props.diffFile.getOldPlainLine(unifiedItem.value.oldLineNumber) : void 0);
  useSubscribeDiffFile(props, (diffFile) => {
    var _a2, _b2, _c2, _d2;
  ...
  return () => {
    ...
    if (currentItemHasChange.value) {
      if (unifiedItem.value.oldLineNumber) {
        return vue.createVNode(DiffUnifiedOldLine, {
        ...
        return vue.createVNode(DiffUnifiedNewLine, {
          "index": props.lineNumber,
          "enableWrap": enableWrap.value,
          "diffFile": props.diffFile,
          "rawLine": unifiedItem.value.value || "",
      ...
      return vue.createVNode("tr", {
      ...
      }, [enableAddWidget.value && unifiedItem.value.diff && vue.createVNode(DiffUnifiedAddWidget, {
        "index": props.index,
        "diffFile": props.diffFile,
        "lineNumber": unifiedItem.value.newLineNumber,
        "side": core.SplitSide.new,
        ...
        "class": "inline-block w-[50%]"
      }, [unifiedItem.value.oldLineNumber]), vue.createVNode("span", {
        "class": "w-[10px] shrink-0"
      }, null), vue.createVNode("span", {
        "data-line-new-num": unifiedItem.value.newLineNumber,
        "class": "inline-block w-[50%]"
      }, [unifiedItem.value.newLineNumber])])]), vue.createVNode("td", {
        "class": "diff-line-content pr-[10px] align-top",
        "style": {
...
const DiffUnifiedExtendLine = /* @__PURE__ */ vue.defineComponent((props) => {
  const extendData = useExtendData();
  const slots = useSlots();
  const enableWrap = useEnableWrap();
  const unifiedItem = vue.computed(() => props.diffFile.getUnifiedLine(props.index));
  const oldExtend = vue.computed(() => {
    var _a, _b;
    return (_b = (_a = extendData.value) == null ? void 0 : _a.oldFile) == null ? void 0 : _b[unifiedItem.value.oldLineNumber];
  });
  const newExtend = vue.computed(() => {
    var _a, _b;
    return (_b = (_a = extendData.value) == null ? void 0 : _a.newFile) == null ? void 0 : _b[unifiedItem.value.newLineNumber];
  });
  const currentIsHidden = vue.computed(() => unifiedItem.value.isHidden);
  const currentIsShow = vue.computed(() => Boolean((oldExtend.value || newExtend.value) && !currentIsHidden.value && slots.extend));
  const width = useDomWidth({
    selector: vue.ref(".unified-diff-table-wrapper"),
    enable: currentIsShow
  });
  return () => {
    if (!currentIsShow.value) return null;
...
const DiffUnifiedHunkLine = /* @__PURE__ */ vue.defineComponent((props) => {
  const currentHunk = vue.computed(() => props.diffFile.getUnifiedHunkLine(props.index));
  const enableExpand = vue.computed(() => props.diffFile.getExpandEnabled());
  const couldExpand = vue.computed(() => enableExpand.value && currentHunk.value && currentHunk.value.unifiedInfo);
  const enableWrap = useEnableWrap();
  const currentIsShow = vue.ref(currentHunk.value && currentHunk.value.unifiedInfo && currentHunk.value.unifiedInfo.startHiddenIndex < currentHunk.value.unifiedInfo.endHiddenIndex);
  const currentIsEnableAll = vue.ref(currentHunk.value && currentHunk.value.unifiedInfo && currentHunk.value.unifiedInfo.endHiddenIndex - currentHunk.value.unifiedInfo.startHiddenIndex < core.composeLen);
  const currentIsFirstLine = vue.computed(() => currentHunk.value && currentHunk.value.isFirst);
  const currentIsLastLine = vue.computed(() => currentHunk.value && currentHunk.value.isLast);
  const currentIsPureHunk = vue.computed(() => currentHunk.value && props.diffFile._getIsPureDiffRender() && !currentHunk.value.unifiedInfo);
  useSubscribeDiffFile(props, () => {
    ...
    currentIsEnableAll.value = currentHunk.value && currentHunk.value.unifiedInfo && currentHunk.value.unifiedInfo.endHiddenIndex - currentHunk.value.unifiedInfo.startHiddenIndex < core.composeLen;
  });
  return () => {
    var _a;
    if (!currentIsShow.value && !currentIsPureHunk.value) return null;
    return vue.createVNode("tr", {
      ...
      "className": "fill-current"
    }, null)]) : currentIsEnableAll.value ? vue.createVNode("button", {
      "class": "diff-widget-tooltip flex w-full cursor-pointer items-center justify-center rounded-[2px] py-[6px]",
      "title": "Expand All",
      "data-title": "Expand All",
...
const DiffUnifiedWidgetLine = /* @__PURE__ */ vue.defineComponent((props) => {
  const slots = useSlots();
  const widget = useWidget();
  const setWidget = useSetWidget();
  const enableWrap = useEnableWrap();
  const unifiedItem = vue.computed(() => props.diffFile.getUnifiedLine(props.index));
  const oldWidget = vue.computed(() => {
    var _a;
    return ((_a = unifiedItem.value) == null ? void 0 : _a.oldLineNumber) && widget.value.side === core.SplitSide.old && widget.value.lineNumber === unifiedItem.value.oldLineNumber;
  });
  const newWidget = vue.computed(() => {
    var _a;
    return ((_a = unifiedItem.value) == null ? void 0 : _a.newLineNumber) && widget.value.side === core.SplitSide.new && widget.value.lineNumber === unifiedItem.value.newLineNumber;
  });
  const currentIsHidden = vue.computed(() => unifiedItem.value.isHidden);
  const currentIsShow = vue.computed(() => (oldWidget.value || newWidget.value) && !currentIsHidden.value && !!slots.widget);
  const onCloseWidget = () => setWidget({});
  const width = useDomWidth({
    selector: vue.ref(".unified-diff-table-wrapper"),
    enable: currentIsShow
...
const DiffUnifiedView = /* @__PURE__ */ vue.defineComponent((props) => {
  const lines = vue.ref(core.getUnifiedContentLine(props.diffFile));
  const maxText = vue.computed(() => Math.max(props.diffFile.unifiedLineLength, props.diffFile.fileLineLength).toString());
  useSubscribeDiffFile(props, (diffFile) => {
    lines.value = core.getUnifiedContentLine(diffFile);
  });
  const fontSize = useFontSize();
  const enableWrap = useEnableWrap();
  const selectState = {
...
#${id} [data-state="widget"] {user-select: none}`;
  ...
  const computedWidth = vue.computed(() => Math.max(40, width.value + 10));
  return () => {
    return vue.createVNode("div", {
      ...
      "scope": "col"
    }, [vue.createTextVNode("line number")]), vue.createVNode("th", {
      "scope": "col"
    }, [vue.createTextVNode("line content")])])]), vue.createVNode("tbody", {
      "class": "diff-table-body leading-[1.6]",
      "onMousedown": onMouseDown
    }, [lines.value.map((item) => vue.createVNode(vue.Fragment, {
      "key": item.index
    }, [vue.createVNode(DiffUnifiedHunkLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffUnifiedContentLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffUnifiedWidgetLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null), vue.createVNode(DiffUnifiedExtendLine, {
      "index": item.index,
      "lineNumber": item.lineNumber,
      "diffFile": props.diffFile
    }, null)])), vue.createVNode(DiffUnifiedHunkLine, {
...
const DiffView = /* @__PURE__ */ vue.defineComponent((props, options) => {
  ...
  const enableHighlight = vue.computed(() => props.diffViewHighlight ?? true);
  const theme = vue.computed(() => props.diffViewTheme);
  vue.watch(() => props.diffFile, () => {
    var _a2, _b2;
  ...
  vue.watch(() => props.initialWidgetState, () => {
  ...
  vue.watch(() => diffFile.value, () => widgetState.value = {});
  const isMounted = useIsMounted();
  const initSubscribe = (onClean) => {
  ...
  vue.watchEffect((onClean) => initSubscribe(onClean));
  vue.watchEffect(() => initDiff());
  vue.watchEffect(() => initSyntax());
  vue.watchEffect((onClean) => initId(onClean));
  vue.watchEffect((onClean) => initAttribute(onClean));
  vue.provide(idSymbol, id);
  vue.provide(domSymbol, wrapperRef);
  vue.provide(mountedSymbol, isMounted);
  vue.provide(slotsSymbol, options.slots);
  vue.provide(onAddWidgetClickSymbol, options.emit);
  vue.provide(widgetStateSymbol, widgetState);
  vue.provide(setWidgetStateSymbol, setWidget);
  useProvide(props, "diffViewMode", modeSymbol, {
    defaultValue: exports.DiffModeEnum.SplitGitHub
  });
  ...
  return () => {
    ...
    return vue.createVNode("div", {
      ...
      "style": props.style
    }, [!props.diffViewMode || props.diffViewMode & exports.DiffModeEnum.Split ? vue.createVNode(DiffSplitView, {
      "key": exports.DiffModeEnum.Split,
      "diffFile": diffFile.value
    }, null) : vue.createVNode(DiffUnifiedView, {
      "key": exports.DiffModeEnum.Unified,
      "diffFile": diffFile.value
    }, null)])])]);
  };
}, {
...
//# sourceMappingURL=vue-git-diff-view.cjs.map
